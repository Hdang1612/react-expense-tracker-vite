...
const ExpenseItem = ({ transaction, updateAction, openItemId, setOpenItemId, categoryName }) => {
    ...
    <div>
      ...
      >
        <div className='flex flex-row items-center cursor-pointer' onClick={handleToggleDropdown}>
          ...
          <div className='ml-3'>
            <p className='text-[16px] md:text-2xl font-regular text-[#000000]'>{categoryName}</p>
          ...
          <div className='flex mt-2 bg-transparent p-4 rounded-md shadow-lg w-full'>
            <div>
              <p className='font-bold mb-2'>
                {transaction.transactionType === 'income' ? 'Income Receipt' : 'Expense Receipt'}
              </p>
              {transaction.receipt ? (
                <img
      ...
      <Modal
        ...
        className=''
      >
        <img src={`${BASE_PATH}${transaction.receipt}`} alt='Receipt' className='' />
      </Modal>
    </div>
  )
}
