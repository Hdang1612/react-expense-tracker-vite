  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(deleteCategory.fulfilled, (state, action) => {
        ...
        state.transactions = state.transactions.filter(
          (transaction) => transaction.transactionCategory !== action.payload.id
        )
...
const updateTotalBalance = (state) => {
  ...
  const income = transactions
    ...
    .reduce((acc, transaction) => acc + Number(transaction.transactionAmount), 0)
...
export const selectTodayTransactions = (state) => {
  ...
  if (state.transactions.transactions) {
    return state.transactions.transactions.filter((transaction) => {
      const transactionDate = new Date(transaction.createAt).toLocaleDateString('en-GB')
